...
- `ID` — internal identifier number for each molecule.
- `MolecularMass` — molecular mass read from Gaussian output, float scalar in atomic mass units _u_.
...
- `Multiplicity` — spin multiplicity  
  \( M_S = 2S - 1 \)  
  where \(S\) is total electronic spin. Computed by RDKit assuming high-spin for unpaired electrons from `GetNumRadicalElectrons`. Multiplicity of oxygen set to 3.
...
- `QuadrupoleMoment` — scalar magnitude of quadrupole matrix from Gaussian in **D·Å**, computed from traceless quadrupole diagonal elements  
  \( Q = \frac{2}{3}\left[(Q^\mathrm{tl}_{xx})^2 + (Q^\mathrm{tl}_{yy})^2 + (Q^\mathrm{tl}_{zz})^2\right] \).
...
- `PolarIso` — isotropic polarizability average  
  \( \alpha*{\text{iso}} = \frac{1}{3}(\alpha*{xx} + \alpha*{yy} + \alpha*{zz}) \),  
  float in **(a₀)³**.
- `PolarIso_SI` — isotropic polarizability average in **C²·m²·J⁻¹**.
- `PolarIso_esu` — isotropic polarizability average in **cm³**.
- `PolarAniso` — polarizability anisotropy  
  \( \alpha*{\text{aniso}} = \sqrt{\frac{1}{2}\left[(\alpha*{xx}-\alpha*{yy})^2 + (\alpha*{yy}-\alpha*{zz})^2 + (\alpha*{zz}-\alpha\_{xx})^2\right]} \),  
  float in **(a₀)³**.
